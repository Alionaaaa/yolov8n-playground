#include "../tracker_include/OCSort.hpp"
#include "../tracker_include/ORB.hpp"
#include "iomanip"
#include <utility>
...
namespace ocsort {
    ...
    OCSort::OCSort(float det_thresh_, int max_age_, int min_hits_, float iou_threshold_, int delta_t_, 
    std::string asso_func_, float inertia_, int max_kalman_, bool use_byte_)  {
        max_age = max_age_;
        min_hits = min_hits_;
    ...
    std::vector<Eigen::RowVectorXf> OCSort::update(Eigen::MatrixXf dets) {
        ...
        for (int i = trackers.size() - 1; i >= 0; i--) {
            ...
            if ((trackers.at(i).time_since_update == 0 || (trackers.at(i).time_since_update > 0 && trackers.at(i).time_since_update < max_kalman)) && ((trackers.at(i).hit_streak >= min_hits) | (frame_count <= min_hits))) {
                Eigen::RowVectorXf tracking_res(8);
                tracking_res << d(0), d(1), d(2), d(3), trackers.at(i).id + 1, trackers.at(i).cls, trackers.at(i).conf, trackers.at(i).time_since_update == 0;
